...
func (s *SettingsService) ListManualOrderSources(tenantID uint64, enabledOnly bool) ([]model.ManualOrderSource, error) {
	return s.repos.ListManualOrderSources(tenantID, enabledOnly)
}

func (s *SettingsService) CreateManualOrderSource(tenantID uint64, name, code, remark string, sort int, enabled bool) (*model.ManualOrderSource, error) {
	name = strings.TrimSpace(name)
	if name == "" {
		return nil, fmt.Errorf("来源名称不能为空")
	}
	row := &model.ManualOrderSource{
		TenantID: tenantID,
		Name:     name,
	...
	if err := s.repos.CreateManualOrderSource(row); err != nil {
		if strings.Contains(err.Error(), "duplicate") || strings.Contains(err.Error(), "UNIQUE") || strings.Contains(err.Error(), "uk_manual_src") {
			return nil, fmt.Errorf("来源名称「%s」已存在", name)
		}
		return nil, err
	}
	return row, nil
...
func (s *SettingsService) UpdateManualOrderSource(tenantID, id uint64, name, code, remark *string, sort *int, enabled *bool) (*model.ManualOrderSource, error) {
	row, err := s.repos.GetManualOrderSource(tenantID, id)
	if err != nil {
	...
	if err := s.repos.SaveManualOrderSource(row); err != nil {
		if strings.Contains(err.Error(), "duplicate") || strings.Contains(err.Error(), "UNIQUE") || strings.Contains(err.Error(), "uk_manual_src") {
			return nil, fmt.Errorf("来源名称「%s」已存在", row.Name)
		}
		return nil, err
	}
	return row, nil
...
func (s *SettingsService) DeleteManualOrderSource(tenantID, id uint64) error {
	return s.repos.DeleteManualOrderSource(tenantID, id)
}
