...
type Order struct {
	...
	// SellerFlag 卖家备注旗帜：0灰 1红 2黄 3绿 4蓝 5紫（对齐快递助手/淘宝 star）
	SellerFlag    int        `gorm:"default:0;index" json:"sellerFlag"`
	// ShipContent 发货内容（手工单无商品时写入；有商品时通常为空，打印面单再按规格填充）
	ShipContent   string     `gorm:"type:text" json:"shipContent"`
