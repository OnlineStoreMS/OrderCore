...
func (h *SettingsHandlers) ListManualOrderSources(c *gin.Context) {
	enabledOnly := c.Query("enabledOnly") == "1" || c.Query("enabledOnly") == "true"
	list, err := h.settings.ListManualOrderSources(authcontext.TenantID(c), enabledOnly)
	if err != nil {
		response.Fail(c, http.StatusInternalServerError, err.Error())
		return
	}
...
type manualOrderSourceBody struct {
	Name    string `json:"name"`
...
func (h *SettingsHandlers) CreateManualOrderSource(c *gin.Context) {
	var body manualOrderSourceBody
	if err := c.ShouldBindJSON(&body); err != nil {
		response.Fail(c, http.StatusBadRequest, err.Error())
		return
	}
	...
	row, err := h.settings.CreateManualOrderSource(authcontext.TenantID(c), body.Name, body.Code, body.Remark, body.Sort, enabled)
	if err != nil {
		response.Fail(c, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *SettingsHandlers) UpdateManualOrderSource(c *gin.Context) {
	id, err := parseID(c.Param("id"))
	if err != nil {
		response.Fail(c, http.StatusBadRequest, "无效 ID")
		return
	...
	row, err := h.settings.UpdateManualOrderSource(authcontext.TenantID(c), id, body.Name, body.Code, body.Remark, body.Sort, body.Enabled)
	if err != nil {
		response.Fail(c, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *SettingsHandlers) DeleteManualOrderSource(c *gin.Context) {
	id, err := parseID(c.Param("id"))
	if err != nil {
		response.Fail(c, http.StatusBadRequest, "无效 ID")
		return
	}
	if err := h.settings.DeleteManualOrderSource(authcontext.TenantID(c), id); err != nil {
		response.Fail(c, http.StatusBadRequest, err.Error())
		return
	}
	response.OK(c, gin.H{"ok": true})
